...
public int[] Run(int[] inputs, int column) {
    
    //doesn't quite work yet
    inputs = AddWeights(inputs, column, 0);
    ...
    return Run(inputs, column);
}

private int[] AddWeights(int[] inputs, int column, int weightNum) {
    char c = net[charInString];
    charInString++;
    ...
    return AddWeights(inputs, column, weightNum++);
}
